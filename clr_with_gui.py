...
def closure(items):							#just make new produces sentences, items= closure
	...
	def exists(newitem, items):				#just check if newitem is in items with same lookahead
		for i in items:
...
def goto(items, symbol):					# move the ' . ' one further and then call closure
...
def calc_states():				 # iterates into states and and with every terminal or nonTerminal goes to another state
	global final
	def contains(states, t):     # check if states containts t
...
def make_table(states):
	...
	def getstateno(t):								#returns the number of states
	...
	def getprodno(closure):							#returns the number of closure in production_list
	...
	# print(states) #[['Z->.S', 'S->.Ab', 'A->.nb'], ['Z->S.'], ['S->A.b'], ['A->n.b'], ['S->Ab.'], ['A->nb.']]
	...
	for s in states:
		...
		for item in s.closure:
			...
			if nextsym=='':								# end of sentence
				if getprodno(item)==0:					# accept
...
if __name__=="__main__":
	main()
	print("production list ",firstfollow.production_list)
	print("final : ",final)
